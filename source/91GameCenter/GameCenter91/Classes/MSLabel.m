...
- (void)drawTextInRect:(CGRect)rect 
{
    ...
    //
    NSMutableArray *tmpArray = [NSMutableArray arrayWithArray:slicedStrings];
    if (tmpArray.count > self.numberOfLines) {
        NSString *lastLine = [tmpArray objectAtIndex:self.numberOfLines - 1];
        NSString *partOfLine = [lastLine substringToIndex:lastLine.length - 3];
        lastLine = [partOfLine stringByAppendingString:@"..."];
        [tmpArray replaceObjectAtIndex:(self.numberOfLines - 1) withObject:lastLine];
    }
    slicedStrings = [NSArray arrayWithArray:tmpArray];
