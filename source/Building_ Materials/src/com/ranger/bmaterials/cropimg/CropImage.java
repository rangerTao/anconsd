...
{
//    public static final File FILE_LOCAL = new File(Constants.IMAGE_PATH,Constants.PHOTO_LOCAL_FILE);
    public static final File FILE_LOCAL = new File(Constants.IMAGE_PATH,Constants.PHOTO_UNCOMMIT_FILE);
	...
	private final static int IMG_SIZE = 300;
	...
	public void startRotate(float d) {
        ...
        showProgressDialog("稍等", new Runnable() {
            public void run() {
...
//        Rect dr = mCrop.mDrawRect;
        int width = IMG_SIZE;//dr.width(); // modify by yc
        int height = IMG_SIZE;//dr.height();
        Bitmap croppedImage = Bitmap.createBitmap(width, height, Bitmap.Config.RGB_565);
        {
            Canvas canvas = new Canvas(croppedImage);
            Rect dstRect = new Rect(0, 0, width, height);
