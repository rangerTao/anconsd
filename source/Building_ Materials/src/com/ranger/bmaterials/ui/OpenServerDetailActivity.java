...
public class OpenServerDetailActivity extends SquareDetailBaseActivity {
	...
	private static final String TAG = "OpenServerDetailActivity";
	protected void loadData() {
		NetUtil.getInstance().requestForOpenServerDetail(gameId, activityId, new RequestContentListener());
		registerListener();
		registerReceiver();
	}
	...
	protected void initTitleBar() {
		...
		titleText.setText(R.string.square_openserver_detail_title);
	}

	@Override
	protected void check() {
		Intent intent = getIntent();
		this.gameId = intent.getStringExtra(ARG_GAME_ID);
		this.activityId = intent.getStringExtra(ARG_OPENSERVER_ID);
	...
	protected void fillData(BaseResult d) {
		try {
			super.fillData(d);
			data = (OpenServerDetail) d;
			if(TextUtils.isEmpty(data.getOpenid())){
				fillEmptyData(R.string.none_openserver_data);
				return;
			}
			titleTv.setText(StringUtil.convertEscapeString(data.getOpentitle()));
			timeTv.setText(DateUtil.formatDate(new Date(data.getInfostime())));
			List<OpenServerItem> list = data.getData();
			int size = list.size();

			String html = "";
			for (int i = 0; i < size; i++) {

				OpenServerItem item = list.get(i);
			...
			if (data.item != null && !TextUtils.isEmpty(gameId = data.item.getGameId())&&(!getPackageName().equals(data.item.getPackageName()))) {
				...
				iconIv.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
                        ...
                        manager.jumpToDetail(OpenServerDetailActivity.this, gameId, gameName, null, false);
                    }
				});
	...
	public void onClick(View v) {
		...
		case R.id.search_item_action_layout:
			onGameItemClick();
			ClickNumStatistics.addSquareOpenServiceDetailDownloadStatistics(this);
			break;
