...
public class ASIHttpRequest implements Runnable {
	...
	private MyLogger mLogger = MyLogger.getLogger(ASIHttpRequest.class
			.getName());
		...
		try {
			...
			if (responseCode == HttpStatus.SC_OK
				...
				if (responseDataLen != -1) {
					...
					MyLogger.getLogger(this.getClass().getName()).v(
				...
				if (responseCode == HttpStatus.SC_GATEWAY_TIMEOUT) {
					...
					String codestr = String.format("Net Error Code: %d",
							responseCode);
					String msgstr = String.format("Net Error Msg: %s", response
