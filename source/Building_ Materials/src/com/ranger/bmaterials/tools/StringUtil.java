...
public class StringUtil {
	...
	public static boolean isChineseChar(String str){
		...
		if (m.find() || str.equals("《") || str.equals("【")){ 
            temp =  true;
        }
        return temp;
    }
	public static String convertHtmlLabel(String string) {
		if (TextUtils.isEmpty(string))
			return "";
		return string.replace("[BR]", "<br/>").replace("[IMG]", "<img src=\"").replace("[/IMG]", "\"/>");
	}
	/**
	 * 
	 * @param string
	 * @return
	 */
	public static String convertEscapeStringWithNoCharacter(String string) {
		if (TextUtils.isEmpty(string))
			return "";
		String convertEscapeString = convertEscapeString(string);
		int index=0;
		while (!isChineseChar(convertEscapeString.charAt(index)+"")) {
			index++;
		}
		return convertEscapeString.substring(index);
	}
	/**
	 * 
	 * @param string
