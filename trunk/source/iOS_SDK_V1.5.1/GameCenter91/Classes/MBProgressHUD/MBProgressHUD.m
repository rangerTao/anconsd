...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated title:(NSString *)title message:(NSString *)message
{
	MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view];
    hud.labelText = title;
...
- (id)initWithView:(UIView *)view {
	...
	((UIView *)me).userInteractionEnabled = NO;
	return me;
}
...
@implementation MBProgressHUD(Block)

+ (MBProgressHUD *)showBlockHUD:(BOOL)animated
{
    UIWindow *win = [[[UIApplication sharedApplication] windows] objectAtIndex:0];
    MBProgressHUD *hud = [self showHUDAddedTo:win animated:animated];
    hud.userInteractionEnabled = YES;
    return hud;
}

+ (MBProgressHUD *)showBlockHUD:(NSString *)title message:(NSString *)message
{
    UIWindow *win = [[[UIApplication sharedApplication] windows] objectAtIndex:0];    
...
+ (BOOL)hideBlockHUD:(BOOL)animated
{
    UIWindow *win = [[[UIApplication sharedApplication] windows] objectAtIndex:0];
    return [self hideHUDForView:win animated:animated];
}

@end

@implementation MBProgressHUD(Hint)
+ (MBProgressHUD *)showHintHUD:(NSString *)title message:(NSString *)message hideAfter:(CGFloat)seconds
{
    UIWindow *win = [[[UIApplication sharedApplication] windows] objectAtIndex:0];
    UIView *lastHUD = [win viewWithTag:kShowHintHUDTag];
    if (lastHUD) {
        [lastHUD removeFromSuperview];
    }
    
	MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:win];
    hud.tag = kShowHintHUDTag;
    hud.customView = nil;
    hud.margin = 10;
    hud.mode = MBProgressHUDModeCustomView;
    hud.labelFont = [UIFont boldSystemFontOfSize:14];
    hud.detailsLabelFont = [UIFont boldSystemFontOfSize:LABELDETAILSFONTSIZE];    
    hud.labelText = title;
    hud.detailsLabelText = message;
    [win addSubview:hud];
    [hud show:YES];
...
+ (MBProgressHUD *)showAndroidToast:(NSString *)message hideAfter:(CGFloat)seconds
{
    UIWindow *win = [[[UIApplication sharedApplication] windows] objectAtIndex:0];    
	MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:win];
    hud.customView = nil;
    ...
    hud.mode = MBProgressHUDModeCustomView;
    hud.labelText = @"";
    hud.detailsLabelFont = [UIFont boldSystemFontOfSize:LABELFONTSIZE];    
    hud.detailsLabelText = message;
    [win addSubview:hud];
    [hud show:YES];
    [hud hide:YES afterDelay:seconds];
