...
#import "sqlite3.h"
#import "NdFMResultSet.h"

@interface NdFMDatabase : NSObject 
{
	sqlite3*    db;
...
- (NdFMResultSet *)executeQuery:(NSString*)sql, ...;
- (NdFMResultSet *)executeQueryWithFormat:(NSString*)format, ...;
- (NdFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
- (NdFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args; // you shouldn't ever need to call this.  use the previous two instead.
...
@interface NdFMStatement : NSObject {
