...
import com.ranger.bmaterials.obverser.PackageSizeObverser;
...
public class PackageUtils {
	...
	/**
 	 ...
 	 * @return
 	 */
	public static CommandResult installSilent(String apkAbsolutePath ){  
...
*/
    ...
    public static int installSilent(Context context, String filePath) {
        ...
        if (commandResult.successMsg != null
        	...
        	Log.i("SilentInstallService", "Success");
        ...
        if (commandResult.errorMsg != null) {
        	Log.i("SilentInstallService", "error msg:"+commandResult.errorMsg);
     ...
     * @throws Exception
     */
    public static PackageSizeInfo queryInstalledPacakgeSize(Context context,String pkgName){
        if ( pkgName != null){
            //使用放射机制得到PackageManager类的隐藏函数getPackageSizeInfo

            PackageManager pm = context.getPackageManager();  //得到pm对象

            PackageSizeObverser pso = new PackageSizeObverser();
            try {
                //通过反射机制获得该隐藏函数
...
//                if(pso.getPackageSizeInfo().isSuccessed()){
//                    pso.getPackageSizeInfo().setPkgname(pkgName);
//
//                    return pso.getPackageSizeInfo();
//                }
            }
            catch(Exception ex){
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				if (newPs != null) {
					infoString += "codesize: " + formatFileSize(newPs.codeSize);
					infoString += "\ndatasize: "+ formatFileSize(newPs.dataSize);
					infoString += "\ncachesize" + formatFileSize(newPs.cacheSize);
				}
	...
	public void getpkginfo(Context context,String pkg,Message msg) {
		PackageManager pm = context.getPackageManager();
		this.mMessage = msg;
		try {
			Method getPackageSizeInfo = pm.getClass().getMethod("getPackageSizeInfo", String.class,IPackageStatsObserver.class);
    ...
    public void getpkginfoAsync(Context context,String pkg, IPackageStatsObserver.Stub stub) {
        PackageManager pm = context.getPackageManager();
        try {
            Method getPackageSizeInfo = pm.getClass().getMethod("getPackageSizeInfo", String.class,IPackageStatsObserver.class);
            getPackageSizeInfo.invoke(pm, pkg, stub);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//			Message msg = mHandler.obtainMessage(1);
            ...
            data.putString(Constants.JSON_PKGNAME,pkgname);
//			msg.setData(data);
