...
public class AppraisalDetailActivity extends SquareDetailBaseActivity {
	...
	private AppiaisalDetail data;
	private ImageView search_item_action_iv;
	private ImageView search_item_comingsoon_iv;
	private TextView search_item_action_tv;
	private static final String TAG = "AppraisalDetailActivity";
	private boolean isfromrec;

	protected void loadData() {
		NetUtil.getInstance().requestForAppraisalDetail(activityId, new RequestContentListener());
		registerListener();
		registerReceiver();
	}
	...
	protected void initTitleBar() {
		...
		if (pageId == SquareSubPageFragment.PAGE_APPRAISAL) {
			titleText.setText(R.string.square_apraisal_detail_title);
		} else {
			titleText.setText(R.string.square_information_detail_title);
		}
	}
	...
	protected void check() {
		...
		info = (ActivityInfo) intent.getSerializableExtra(SquareDetailBaseActivity.ARG_DETAIL);
		pageId = intent.getIntExtra(SquareSubPageFragment.ARG_CURRENT_PAGE, -1);
		activityId = info.getId();
		isfromrec = intent.getBooleanExtra("isfromrec", false);
		if (isfromrec) {
			MainHallActivity.JUMP_TO_TAB_EXTRA = 3;
		}
		if (info == null || pageId == -1 || activityId == null) {
			finish();
		}
	}
	@Override
	protected void fillData(BaseResult d) {
		try {
			super.fillData(d);
			data = (AppiaisalDetail) d;
//			if(TextUtils.isEmpty(data.get))
			titleTv.setText(TextUtils.isEmpty(info.getTitle())?data.getInfostitle():info.getTitle());
			timeTv.setText(DateUtil.formatDate(new Date(data.getInfostime())));
			appraisal_source_text.setText(data.getInfossource());
			detailLayout.loadDataWithBaseURL(null, StringUtil.convertHtmlLabel(data.getInfoscontent()), "text/html", "UTF-8", null);
			if (data.item != null && !TextUtils.isEmpty(gameId = data.item.getGameId())&&(!getPackageName().equals(data.item.getPackageName()))) {
				ImageLoaderHelper.displayImage(data.item.getIconUrl(), iconIv);
				...
				iconIv.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
                        ...
                        manager.jumpToDetail(AppraisalDetailActivity.this, gameId, gameName, null, false);
                    }
				});

				appraisal_title_text.setText(data.item.getGameName());
				appraisal_version_text.setText("版本  " + data.item.getVersion());
	...
	public void onClick(View v) {
		...
		case R.id.search_item_action_layout:
			onGameItemClick();
			if(pageId == SquareSubPageFragment.PAGE_APPRAISAL){
				ClickNumStatistics.addSquareInformationDetailDownloadStatistics(this);
			}else{
				ClickNumStatistics.addSquareAppraisalDetailDownloadStatistics(this);
			}
			break;
		default:
			super.onClick(v);
...
//	@Override
//	public int emptyDataHint() {
//		if (pageId == SquareSubPageFragment.PAGE_APPRAISAL) {
//			return R.string.none_appraisal_data;
//		} else
//			return R.string.none_information_data;
//	}
}
