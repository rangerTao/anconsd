...
import com.ranger.bmaterials.download.Downloads.Impl;
...
public final class DownloadInfo {
	...
	public static class Reader {
		...
		public void updateFromDatabase(DownloadInfo info) {
			synchronized (info) { // add by caohaitao fix SEARHBOX-65
				...
				String data = getString(info.mFileName, Downloads.Impl.DATA);
				info.mFileName = TextUtils.isEmpty(data) ? null : data;
				...
				int dbStatus = getInt(Downloads.Impl.COLUMN_STATUS);
				// ....
				// controlû�䣬status�仯 ��
				if ((info.mStatus == Downloads.STATUS_PENDING || info.mStatus == Downloads.STATUS_RUNNING)
						&& (dbStatus == Downloads.Impl.STATUS_PAUSED_BY_APP
								|| dbStatus == Downloads.Impl.STATUS_WAITING_TO_RETRY
								|| dbStatus == Downloads.Impl.STATUS_WAITING_FOR_NETWORK || dbStatus == Downloads.Impl.STATUS_QUEUED_FOR_WIFI)) {
					info.mPauseNotiModifyFlag = false;
				}
				// ........
				info.mStatus = dbStatus;

				info.mControl = dbControl;

				// added by wangliang
				info.mMarkData = getString(info.mMarkData,
	...
	/** the allowed limit size.�Ƿ������û�ȷ�������ļ����� */
	...
	public void sendIntentIfRequested(int status) {
		 ...
		 */
		/*
		 * try { Intent intent; if (mIsPublicApi) { intent = new
		 * Intent(DownloadManager.ACTION_DOWNLOAD_COMPLETE);
		 * intent.setPackage(mPackage);
		 * intent.putExtra(DownloadManager.EXTRA_DOWNLOAD_ID, mId); } else { //
		 * legacy behavior if (mClass == null) { return; } intent = new
		 * Intent(Downloads.Impl.ACTION_DOWNLOAD_COMPLETED);
		 * intent.setClassName(mPackage, mClass); if (mExtras != null) {
		 ...
		 * malicious // applications would have an easier time spoofing download
		 * results by // sending spoofed intents.
		 * intent.setData(getMyDownloadsUri()); }
		 * mSystemFacade.sendBroadcast(intent); } catch (Exception e) { }
		 */
		Intent intent = null;
		// Log.i("DownloadInfo", "sendIntentIfRequested status:"+status);
		boolean statusSuccess = Downloads.Impl.isStatusSuccess(status);
		if (statusSuccess) {
			...
			intent.putExtra(EXTRA_SUCCESSFUL, true);
				...
				&& status != Downloads.Impl.STATUS_CANCELED) {
			intent = new Intent(DownloadManager.ACTION_DOWNLOAD_COMPLETE);
			intent.putExtra(EXTRA_SUCCESSFUL, false);
			intent.putExtra(EXTRA_REASON, status);
		} else if (status == Downloads.Impl.STATUS_WAITING_TO_RETRY
				|| status == Downloads.Impl.STATUS_WAITING_FOR_NETWORK
				|| status == Downloads.Impl.STATUS_QUEUED_FOR_WIFI) {
			// STATUS_WAITING_TO_RETRY = 194;
			// STATUS_WAITING_FOR_NETWORK = 195;
			// int STATUS_QUEUED_FOR_WIFI = 196;
			intent = new Intent(DownloadManager.ACTION_DOWNLOAD_PAUSE);
			intent.putExtra(EXTRA_REASON, status);
		} else if (status == Downloads.Impl.STATUS_PAUSED_BY_APP) {
			intent = new Intent(DownloadManager.ACTION_DOWNLOAD_PAUSE_BY_USER);
			intent.putExtra(EXTRA_REASON, status);
		} else if (status == Downloads.Impl.STATUS_CANCELED) {
			intent = new Intent(DownloadManager.ACTION_DOWNLOAD_CANCLE);
		}
		if (intent != null) {
	...
	/**
	 * Notify the download thread start.
	 */
	...
	// added by wangliang
	public void sendRunningRequested() {
		if (true) {
			long downloadId = mId;
			String downloadUrl = mUri;
			String saveDest = mHint;
			String title = mTitle;
			String appData = mMarkData;
			long currentSize = mCurrentBytes;
			long totalSize = mTotalBytes;
			DownloadHelper.notifyDownloadProgress(downloadId, downloadUrl,
					saveDest, title, appData, currentSize, totalSize);
			// return ;
		}

		Intent intent = new Intent(DownloadManager.ACTION_DOWNLOAD_RUNNING);
	...
	public static final String EXTRA_CURRENT_SIZE = "current_size";
	...
	private boolean isReadyToStart(long now) {
		...
		case Downloads.Impl.STATUS_PAUSED_BY_APP: // ��ͣ״̬�����ؿ������¿�ʼ
	...
	private boolean isRoamingAllowed() {
		...
		/*
		 * } else { // legacy behavior return mDestination !=
